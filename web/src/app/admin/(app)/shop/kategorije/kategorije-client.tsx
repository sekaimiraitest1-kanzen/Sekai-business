    ...
    <div className="adm-sheet-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="adm-sheet">
        ...
        <div style={{ padding: "0 20px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <input className="adm-input" value={lat} onChange={(e) => { setLat(e.target.value); if (!slug) setSlug(slugify(e.target.value)); }} />
          ...
          {/* English toggle mirrors the Latin name — no separate translation field anymore. */}
          <button className="adm-btn adm-btn-block" disabled={pending || !lat || !slug} onClick={() => start(async () => { await upsertCategory({ id: category?.id, slug, name_sr: lat, name_lat: lat, active }); onClose(); })}>SAČUVAJ</button>

...
export default async function ProizvodiPage() {
  ...
  const [productsRes, catsRes] = await Promise.all([
    sb.from("products").select("id, slug, name, brand, description, price, category, stock, active, image_url, badge, sort_order").eq("salon_id", session.salonId).order("sort_order", { ascending: true }),

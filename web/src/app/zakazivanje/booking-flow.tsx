...
const MONTHS_SR_SHORT = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
const MONTHS_LAT_SHORT = ["jan", "feb", "mar", "apr", "maj", "jun", "jul", "avg", "sep", "okt", "nov", "dec"];
const MONTHS_SR_FULL = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  ...
  return (
    <>
      <h1 className="sr-only">
        <span data-sr>Booking — Barbershop Vuk, Batajnica</span>
      ...
      <nav className="bk-nav">
        ...
        <div className="bk-nav-title" data-sr>BOOKING</div>
              ...
              {services.map((s) => (
                ...
                >
                  <div>
                    ...
                    <div className="service-card-meta" data-sr>{s.duration_min} MIN</div>
                    <div className="service-card-meta" data-lat>{s.duration_min} MIN</div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div className="service-card-price">
                      {s.price}
                      <span data-sr>RSD</span>
          ...
          <div className="step-screen active">
            ...
            <div style={{ marginTop: 8, marginBottom: 24 }}>
              <button className="bk-btn-secondary" onClick={() => setCalOpen(true)} style={{ fontSize: 12, padding: "10px 16px" }}>
                📅 <span data-sr>FULL CALENDAR</span><span data-lat>PUN KALENDAR</span>
              </button>
            </div>
            ...
            <div className="time-grid">
              {slots.length === 0 ? (
                <div style={{ gridColumn: "span 4", textAlign: "center", padding: 32, fontFamily: "'JetBrains Mono', monospace", color: "var(--brown-700)", opacity: 0.6 }}>
                  <span data-sr>No available slots for this day.</span>
          ...
          <div className="step-screen active">
            ...
            {loyaltyReward === "free_cut" && (
              <div style={{ padding: 14, background: "rgba(212,165,58,.18)", borderLeft: "3px solid var(--mustard)", marginBottom: 16, fontSize: 13, lineHeight: 1.55, color: "var(--cream)" }}>
                🎁 <strong>
                  <span data-sr>This appointment is FREE!</span>
                  <span data-lat>Ovaj termin je BESPLATAN!</span>
                </strong>
                <br />
                <span data-sr>You have an active loyalty reward for your 6th visit. This appointment is 0 RSD.</span>
            ...
            {flagged && !loyaltyReward && (
              <div style={{ padding: 14, background: "rgba(204,34,34,.1)", borderLeft: "3px solid #cc2222", marginBottom: 16, fontSize: 13, lineHeight: 1.55, color: "var(--cream)" }}>
                ⚠ <strong>
                  <span data-sr>This appointment has a +30% surcharge</span>
                  <span data-lat>Ovaj termin ima +30% doplate</span>
                </strong>
                <br />
                <span data-sr>Because your previous appointment was cancelled less than 2 hours before, or you didn't show up. The surcharge is paid at the salon. Your next appointment returns to the regular price.</span>
            ...
            <div className="form-group">
              <label className="form-label" data-sr>FULL NAME</label>
              <label className="form-label" data-lat>IME I PREZIME</label>
              <input className="form-input" type="text" placeholder={lang === "lat" ? "Marko Jovanović" : "John Smith"} value={name} onChange={(e) => setName(e.target.value)} />
            </div>

            <div className="form-group">
              <label className="form-label">
                <span data-sr>PHONE NUMBER</span>
              ...
              <p className="form-hint" data-sr>Your phone number is your identity in the system. We never share it.</p>
              <p className="form-hint" data-lat>Telefon je tvoj identitet u sistemu. Nikome ga ne delimo.</p>
            </div>

            <div className="form-group">
              <label className="form-label">
                <span data-sr>EMAIL</span>
                <span data-lat>IMEJL</span>
                <span className="form-optional" data-sr>OPTIONAL</span>
              ...
              <p className="form-hint" data-sr>For booking confirmation and visit history.</p>
            ...
            {submitErr && (
              <div style={{ padding: 12, background: "rgba(166,61,42,.1)", border: "1px solid var(--danger)", color: "var(--danger)", fontFamily: "'JetBrains Mono', monospace", fontSize: 12, marginBottom: 12 }}>
                {submitErr === "SLOT_TAKEN"
                  ? lang === "lat" ? "Termin je upravo zauzet — izaberi drugi." : "This slot was just taken — pick another one."
                  : lang === "lat" ? "Greška pri slanju. Pokušaj ponovo." : "Something went wrong. Please try again."}
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
              ...
              >
                {pending ? (
                  ...
                  <>
                    <span data-sr>CONFIRM BOOKING →</span>
        ...
        {step === 5 && selectedService && date && time && (
          <div className="step-screen active">
            <div className="confirm-card">
              ...
              <h2 className="confirm-title" data-sr>See you soon.</h2>
              <h2 className="confirm-title" data-lat>Vidimo se.</h2>
              <p className="confirm-sub" data-sr>Your booking is confirmed. Free cancellation up to 2 hours before your appointment — after that a 30% penalty applies.</p>
            ...
            <div className="confirm-details">
              ...
              <ConfirmRow labelSr="PRICE" labelLat="CENA" value={`${selectedService.price} ${lang === "lat" ? "RSD" : "RSD"}`} />
              <ConfirmRow labelSr="NAME" labelLat="IME" value={name} />
              <ConfirmRow labelSr="PHONE" labelLat="TELEFON" value={phone} />
              <ConfirmRow labelSr="LOCATION" labelLat="LOKACIJA" value={salonAddress} />
              {bookingId && <ConfirmRow labelSr="ID" labelLat="ID" value={bookingId.slice(0, 8).toUpperCase()} />}
            </div>

            <div className="confirm-policy">
              <p data-sr>⚠ Free cancellation up to 2 hours before your appointment. Later cancellations carry a 30% penalty of the service price.</p>
  ...
  return (
    <div className="summary-bar">
      <div>
        ...
        <div className="summary-bar-meta">{dateLabel ?? `${service.duration_min} ${lang === "lat" ? "MIN" : "MIN"}`}</div>
      </div>
      <div className="summary-bar-price">{service.price} {lang === "lat" ? "RSD" : "RSD"}</div>
    </div>
  );
}
    ...
    <div className="sheet-overlay open" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="sheet">
        <div className="sheet-header">
          <div className="sheet-title" data-sr>CHOOSE DATE</div>
        ...
        <div className="cal-weekdays">
          {(lang === "lat" ? ["PN", "UT", "SR", "ČT", "PT", "SU", "ND"] : ["MO", "TU", "WE", "TH", "FR", "SA", "SU"]).map((w) => (
        ...
        <div className="sheet-confirm-btn">
          <button className="bk-btn-primary" disabled={!picked} onClick={() => picked && onPick(picked)}>
            {picked ? (
              <>
                <span data-sr>CHOOSE {MONTHS_SR_FULL[month].toUpperCase()} {picked.split("-")[2]}</span>
                <span data-lat>IZABERI {picked.split("-")[2]}. {MONTHS_LAT_FULL[month].toUpperCase()}</span>
              </>
            ) : (
              <>
                <span data-sr>CHOOSE DATE</span>

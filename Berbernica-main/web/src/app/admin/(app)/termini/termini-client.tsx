...
import { updateBookingStatus, getDayBookings, deleteBooking, deleteAllCancelledBookings } from "./actions";
...
export function TerminiClient({
  ...
  // Today stats — exclude locally-deleted bookings
  const visibleTodayBookings = todayBookings.filter((b) => !deletedIds.has(b.id));
  const nowStr = new Date().toTimeString().slice(0, 5);
  const upcoming = visibleTodayBookings.filter((b) => (b.status === "confirmed" || b.status === "pending") && b.time_slot.slice(0, 5) >= nowStr);
  const doneToday = visibleTodayBookings.filter((b) => b.status === "done").length;
  const upcomingCount = upcoming.length;
  const revenueToday = visibleTodayBookings
    .filter((b) => b.status === "done")
    .reduce((sum, b) => sum + (unwrap(b.services)?.price ?? 0), 0);
  const nextId = upcoming[0]?.id;
  const cancelledTodayCount = visibleTodayBookings.filter((b) => b.status === "cancelled").length;
    ...
    <>
      ...
      {view === "today" && (
        <>
          ...
          <div className="trm-eyebrow" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span>
              <span data-sr>— РАСПОРЕД</span>
              <span data-lat>— RASPORED</span>
            </span>
            {!isStaffView && cancelledTodayCount > 0 && (
              <button
                type="button"
                disabled={bulkDeleting}
                onClick={async () => {
                  if (!confirm(`Obrisati svih ${cancelledTodayCount} otkazanih termina? Ova akcija čisti analitiku.`)) return;
                  setBulkDeleting(true);
                  await deleteAllCancelledBookings();
                  setDeletedIds(prev => {
                    const next = new Set(prev);
                    todayBookings.filter(b => b.status === "cancelled").forEach(b => next.add(b.id));
                    return next;
                  });
                  setBulkDeleting(false);
                }}
                style={{ fontSize: 11, fontFamily: "'Oswald', sans-serif", letterSpacing: ".06em", textTransform: "uppercase", color: "var(--danger, #c0392b)", background: "none", border: "none", cursor: "pointer", opacity: bulkDeleting ? 0.5 : 1 }}
              >
                <span data-sr>🗑 ОБРИШИ ОТКАЗАНЕ ({cancelledTodayCount})</span>
                <span data-lat>🗑 OBRIŠI OTKAZANE ({cancelledTodayCount})</span>
              </button>
            )}
          </div>
...
// ── DETAIL SHEET ──────────────────────────────────
function BookingDetailSheet({ booking, isNext, pending, isOwner, visitCount, onClose, onAction, onDelete }: {
  booking: Booking;
  isNext: boolean;
  pending: boolean;
  isOwner: boolean;
          ...
          {!isUpcoming && (
            <>
              ...
              {isOwner && booking.status === "cancelled" && (
                <button
                  type="button"
                  className="trm-action-secondary danger"
                  disabled={pending}
                  style={{ marginTop: 10, width: "100%" }}
                  onClick={() => {
                    if (!confirm("Trajno obrisati ovaj termin? Neće se prikazivati u analitici.")) return;
                    onDelete();
                  }}
                >
                  🗑 <span data-sr>ОБРИШИ ТЕРМИН</span><span data-lat>OBRIŠI TERMIN</span>
                </button>
              )}
            </>
          )}

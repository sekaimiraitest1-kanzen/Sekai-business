...
> Generated: 2026-04-29T16:02:14.320Z
...
|------|------|-------------|-----|
| web/scripts/seed-admin.mjs:0 | GIT_HISTORY_SECRET | Secret was removed from code but still exists in git history (commit b19e0b1d04, | Rotate this credential immediately, then clean history: npx  |
...
| web/src/app/admin/(app)/galerija/galerija-client.tsx:27 | API_UPLOAD_NO_TYPE_CHECK | File upload using original filename without type validation. | Validate file extension and MIME type. Generate random filen |
| web/src/app/admin/(app)/galerija/galerija-client.tsx:28 | API_UPLOAD_NO_TYPE_CHECK | File upload using original filename without type validation. | Validate file extension and MIME type. Generate random filen |
...
| web/scripts/seed-admin.mjs:49 | PII_IN_CONSOLE_LOG | PII fields logged to console. Log output may be stored in log aggregation servic | Remove PII from log statements. Use structured logging with  |
| web/scripts/seed-admin.mjs:86 | PII_IN_CONSOLE_LOG | PII fields logged to console. Log output may be stored in log aggregation servic | Remove PII from log statements. Use structured logging with  |
| web/src/app/shop/actions.ts:91 | PII_IN_CONSOLE_LOG | PII fields logged to console. Log output may be stored in log aggregation servic | Remove PII from log statements. Use structured logging with  |
| web/src/app/zakazivanje/actions.ts:117 | PII_IN_CONSOLE_LOG | PII fields logged to console. Log output may be stored in log aggregation servic | Remove PII from log statements. Use structured logging with  |

## Medium (7)

| File | Rule | Description | Fix |
|------|------|-------------|-----|
| web/scripts/seed-admin.mjs:40 | Password Assignment | Hardcoded passwords are a critical vulnerability. | Move to environment variable or secrets manager |
...
| web/scripts/seed-admin.mjs:57 | RAG_SYSTEM_DOCS_WITH_USER_DOCS | System/internal documents mixed with user documents in the same collection. User | Separate system documents from user documents in different c |
| web/scripts/seed-admin.mjs:26 | PII_EMAIL_HARDCODED | Real email address hardcoded in source code (not @example.com). May be PII or a  | Use @example.com for test emails. Store real emails in envir |
| web/src/app/page.tsx:373 | PII_EMAIL_HARDCODED | Real email address hardcoded in source code (not @example.com). May be PII or a  | Use @example.com for test emails. Store real emails in envir |
